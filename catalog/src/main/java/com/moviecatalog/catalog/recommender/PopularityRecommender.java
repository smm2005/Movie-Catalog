...
public class PopularityRecommender implements Recommender{
    ...
    public Float getMetric(){
        List<Float> popularities = getMovies()
        .stream()
        .map(movie -> movie.getPopularity())
        .collect(Collectors.toList());
        float total = 0;
        for (Float popularity: popularities){
            total = total + popularity;
        }
        return (float) (Math.round((total / popularities.size()) * 10.0) / 10.0);
    }

    @Override
    public List<Movie> getRecommendations(){
        List<Movie> movies = movieRepository.findFirst5ByPopularity(getMetric());
